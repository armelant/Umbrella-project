...
const UmbrellasScreen = () => {
  ...
  useEffect(() => {
    axios.get('http://192.168.56.1:5000/umbrellas')
      .then(response => {
  ...
  const availableUmbrellas = umbrellas.filter(item => item.status === 'Available');
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            <Text style={styles.itemText}>Slot number: {item.sensor_id.slice(-2)}</Text>

...
var db = openDatabase({ name: 'historySchema.db' });
var tableName="historySchema";
//method returns a Promise - in the calling side .then(...).then(...)....catch(...) can be used
export const init=()=>{
    const promise=new Promise((resolve, reject)=>{
        db.transaction((tx)=>{
            tx.executeSql('DROP TABLE IF EXISTS History', []); //uncomment this if needed - sometimes it is good to empty the table
            //By default, primary key is auto_incremented - we do not add anything to that column
            tx.executeSql('create table if not exists '+tableName+'(umbrella_id integer not null primary key, rented_at date not null, rented_until date not null, user_id string not null);',
...
export const addRENTAL=(umbrella_id, rented_at, rented_until, user_id)=>{
    const promise=new Promise((resolve, reject)=>{
        db.transaction((tx)=>{
            //Here we use the Prepared statement, just putting placeholders to the values to be inserted
            tx.executeSql('insert into '+tableName+'(umbrella_id, rented_at, rented_until, user_id) values(?,?,?,?);',
            //And the values come here
            [umbrella_id, rented_at, rented_until, user_id],
            //If the transaction succeeds, this is called
            ()=>{
                    resolve();
            },
